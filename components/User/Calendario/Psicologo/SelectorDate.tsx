...
export default function Week() {
  ...
  const handleUpdate = async () => {
    try {
      ...
      if (!storedUser) {
        showToast(
          "error",
          "No se encontró el usuario en el almacenamiento local"
        );
        return;
      }
  ...
  //***************AGREGADO****************** */
//completa los demas campos si es que hay
  ...
  const [historial, setHistorial] = useState<HistorialData[]>([]); //ultimos 7 dias historial
