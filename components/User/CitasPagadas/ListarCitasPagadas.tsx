...
import { TableCitasPagadas } from "./table/TableCitasPagadas";

interface Props {
  filterValue: string;
}

const ListarCitasPagadas = ({ filterValue }: Props) => {
  ...
  const handleGetCitas = useCallback(
    async ({
      ...
      try {
        ...
        const url = `${process.env.NEXT_PUBLIC_API_URL}api/citas/pagadas`;
  ...
  return (
    <>
      {citas.length > 0 ? (
        <>
          <TableCitasPagadas filteredCitas={citas} />
          <Pagination
...
export default ListarCitasPagadas;
