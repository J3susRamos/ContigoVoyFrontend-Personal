...
import { defaultUnaColumna } from "./PlantillasConfig";
...
const EmailMarketingEditor = () => {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      setEmailBlocks(defaultUnaColumna.blocks);
    }
  }, []);
  ...
  const addImageBlock = () => {
    const imageCount = emailBlocks.filter(b => b.type === 'image').length;
    if (imageCount >= 3) {
      showAlert("Solo puedes agregar hasta 3 imágenes.");
      return;
    }
    ...
    setEmailBlocks([...emailBlocks, newBlock]); // se inserta al final
    setSelectedBlock(newBlock.id);
    showAlert(`Imagen ${imageCount + 1} agregada correctamente.`);
  };
            ...
            {showPreview ? (
              ...
              <div className="space-y-4">
                {emailBlocks.map(block => (
                  <div
                    key={block.id}
                    className={`relative bg-gray-700 rounded-lg p-4 cursor-pointer transition-all ${selectedBlock === block.id ? 'ring-2 ring-blue-500' : 'hover:bg-gray-600'
                      }`}
